...
class SNN_Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.alpha1 = torch.nn.Parameter((.001 * torch.rand(1, cfg_fc[0])).cuda(), requires_grad=True)
        self.alpha2 = torch.nn.Parameter((.001 * torch.rand(1, cfg_fc[1])).cuda(), requires_grad=True)
        self.alpha3 = torch.nn.Parameter((.001 * torch.rand(1, cfg_fc[2])).cuda(), requires_grad=True)
    ...
    def forward(self, input, hebb1, hebb2, hebb3, wins=time_window, label=None, mask_dict=None, task_index=None):
        ...
        for step in range(wins):
            ...
            h1_mem, h1_spike, hebb1 = mem_update(self.fc1,  self.alpha1, self.beta1,
                                                 self.gamma1, self.eta1,
                                                 x * decay_factor, h1_spike, h1_mem, hebb1)

            h2_mem, h2_spike, hebb2 = mem_update(self.fc2,  self.alpha2, self.beta2,
...
#
def mem_update(fc, alpha, beta, gamma, eta, inputs, spike, mem, hebb, v_th=thresh):
