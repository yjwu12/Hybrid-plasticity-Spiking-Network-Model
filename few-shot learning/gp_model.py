...
class hybridNet(nn.Module):
    def __init__(self, params=None):
        ...
        # Local parameters
        self.alpha = torch.nn.Parameter((torch.rand(input_dim, nbclasses)).cuda(), requires_grad=True)
        self.beta = torch.nn.Parameter((torch.rand(nbclasses)).cuda(), requires_grad=True)
    ...
    def forward(self, inputs, labels, hebb=None):
        ...
        for step in range(wins):
            ...
            # we can use an encoding layer to convert features into spike trains, but we ignore it here for simplicity.
...
def mem_update_plastic(w, alpha, beta, eta, x, inputlabel, mem, spike, hebb):
    stimulus = x.mm(w.weight.t() ) +  inputlabel
    mem = (1 - spike) * mem * mem_decay + stimulus
    spike = fn2(mem - thr)
